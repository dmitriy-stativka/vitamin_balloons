...
if ( ! function_exists( 'utg_posted_on' ) ) :
	...
	function utg_posted_on() {
		...
		$posted_on = sprintf(
			...
			esc_html_x( 'Posted on %s', 'post date', 'utg' ),
...
if ( ! function_exists( 'utg_posted_by' ) ) :
	/**
	 * Prints HTML with meta information for the current author.
	 */
	function utg_posted_by() {
		$byline = sprintf(
			/* translators: %s: post author. */
			esc_html_x( 'by %s', 'post author', 'utg' ),
	...
	function utg_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			if ( $categories_list ) {
				...
				printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'utg' ) . '</span>', $categories_list ); // WPCS: XSS OK.
			}
			...
			if ( $tags_list ) {
				...
				printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'utg' ) . '</span>', $tags_list ); // WPCS: XSS OK.
			}
		}
